...
[V_1d,t,V0] = figure6_Blackman_potential(x, t_gate);
[X1,X2] = meshgrid(x,x);
t_saves1 = t_gate*[0,0.45,0.55,0.75];
...
%Psi = 0.5*(Psi_left_0.*Psi_right_0+1i*Psi_R_0.*Psi_L_0);
Psi = 0.5*(1i*Psi_left_0.*Psi_right_0+1*Psi_R_0.*Psi_L_0);
Psi = Psi/sqrt(sum(sum(abs(Psi.^2))));
psi_end = psi_end/sqrt(sum(sum(abs(psi_end.^2))));
fidelity = abs(sum(sum(conj(Psi).*psi_end)))^2

fname = 'figure6.pdf';
save_four_in_row_with_cbar3(imgA,imgB,imgC,imgD, ...
    fname,'two');
