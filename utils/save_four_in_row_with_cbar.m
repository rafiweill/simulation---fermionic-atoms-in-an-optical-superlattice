function save_four_in_row_with_cbar(imgA, imgB, imgC, imgD, outpdf, span, opts)
    ...
    function ax = placePanel(c, IMG, tagStr, cornerStr)
        ...
        % Optional scale bar (panel (a) only by default)
%         if c==1 && ~isempty(opts.scaleBarPx) && isnumeric(opts.scaleBarPx)
%             draw_scalebar(ax, opts.scaleBarPx, opts.scaleBarTxt, opts.scaleBarPos, opts);
%         end
    end

    % Panels (a)–(d)
    placePanel(1, IA, '(a)', '$\tau=0$');
    placePanel(2, IB, '(b)', '$\tau_{r}+\frac14\tau_{h}$');
    placePanel(3, IC, '(c)', '$\tau_{r}+\tau_{h}$');
    placePanel(4, ID, '(d)', '$2\tau_{r}+\tau_{h}$');
